...
float DetectBassPeak(fftw_complex* out, int fft_size, float sample_rate, float& cubeScale, float& energyAvg) {
    ...
    energyAvg = smoothing * energyAvg + (1.0f - smoothing) * low_freq_energy;

    // Trigger on peak
    float triggerMultiplier = 1.8f;
    bool peak = low_freq_energy > energyAvg * triggerMultiplier;

    if (peak) {
        cubeScale = 20.0f;
    } else {
        cubeScale -= 0.8f;
        if (cubeScale < 10.0f) cubeScale = 10.0f;
    }
...
int main(int argc, char* argv[]) {
    ...
    while (!interrupt_received) {
        ...
        // Bass peak triggers cube pulse
        cubeScale = DetectBassPeak(fft_output, FFT_SIZE, rate, cubeScale, energyAvg);

        // Hi-hat triggers random color
